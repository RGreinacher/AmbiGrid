...
class BinaryClockAnimation:
    ...
    def __init__(self, device, beVerbose=False):
        # initializations
        self.device = device
        ...
        if self.clockIsRunning:
            try:
                ...
                print('\n', exc_type, fname, exc_tb.tb_lineno,
                      ' - exception while trying to stop the clock timer')
                return False
    ...
    def renderNextFrame(self):
        for bufferIndex in range(0, 25):
            if self.binaryClockBuffer[bufferIndex] == 1:
                ...
                r = self.colors.binaryClockColorRedChannel
                g = self.colors.binaryClockColorGreenChannel
                b = self.colors.binaryClockColorBlueChannel
                index = self.device.pixelMap[bufferIndex]
                self.device.setRgbColorToBufferForLedWithIndex(r, g, b, index)

    def binaryClockTick(self):
        # clear the device buffer if the clock is the only animation to reset
        # each LED after one second
        if self.clockIsOnlyAnimation:
            self.device.clearBuffer()

        time = time = datetime.datetime.now().time()
        time = ["{0:b}".format(time.hour), "{0:b}".format(
            time.minute), "{0:b}".format(time.second)]
        ...
        for timeElement in range(1, 3):
            ...
            for i in range(0, lengthOfBitString):
                if prefixingZeros == 0 and i == 0:
                    bufferPosition = i
                # break line after the first of the six digits
                elif prefixingZeros == 0 and i > 0:
                    bufferPosition = i + 4
                else:
                    bufferPosition = i + 4 + prefixingZeros

                self.binaryClockBuffer[
                    ((timeElement - 1) * 10) + 5 + bufferPosition] = int(time[timeElement][i])
